...
def attention(
    ...
    drop_prob=None, q_time=None, k_time=None, input_mask=False):
    '''Add attention layer.
    Args:
    ...
    if is_mask:
        if input_mask:
            mask_q = tf.expand_dims(Q_lengths, -1)
            mask_v = tf.expand_dims(K_lengths, -1)
            mask = tf.einsum('bik,bjk->bij', mask_q, mask_v)
        else:
...
def block(
    ...
    att = attention(Q, K, V, 
                    ...
                    drop_prob=drop_prob, q_time=q_time, k_time=k_time, input_mask=input_mask)
    if is_layer_norm:
...
def CNN_4d(x, out_channels_0, out_channels_1, add_relu=True):
    ...
    x = tf.nn.convolution(
        input = x,
        filter = tf.get_variable("weights", [3,3,3,3,2,32], tf.float32, tf.truncated_normal_initializer(stddev=0.1)),
        padding = "SAME",
        strides = [1,1,1,1],
        #data_format = "NDHWC"
    )
...
def get_gcn_sent1(sims, adjm, out_dim = 128):
    sim_last = sims
    sim_count = tf.reduce_sum(adjm, axis=1)
    sim_count = tf.expand_dims(sim_count, -1)

    res = []
    for i in range(2):
        #sim_new = tf.matmul(adjm, sim_last)
        sim_new = tf.einsum('bij,ik->bkj', sim_last, adjm)
        sim_new = tf.layers.dense(sim_new, out_dim, kernel_initializer=tf.contrib.layers.xavier_initializer(), name="nei"+str(i))/sim_count
        sim_11 = tf.layers.dense(sim_last, out_dim, kernel_initializer=tf.contrib.layers.xavier_initializer(), name="self"+str(i))
        #sim_last = sim_11
        att_input = tf.concat([sim_new, sim_11], axis=2)
        ...
        sim_last = att_out*sim_new + (1-att_out)*sim_11
        #sim_last = tf.nn.relu(sim_last)
        #sim_last = sim_new + sim_11
        res.append(sim_last)
    return sim_last
    #return res
    #return tf.concat(res, 2)
    #return tf.reduce_mean(sim_last, axis=1)


def get_gnn_sent(sims, adjm, out_dim = 128):
    sim_last = sims
    sim_count = tf.reduce_sum(adjm, axis=1)
    sim_count = tf.expand_dims(sim_count, -1)
    ...
    for i in range(3):
        #sim_new = tf.matmul(adjm, sim_last)
        sim_new = tf.einsum('bij,ik->bkj', sim_last, adjm)/sim_count
        sim_new = tf.layers.dense(sim_new, out_dim, kernel_initializer=tf.contrib.layers.xavier_initializer(), name="nei"+str(i))
        sim_11 = tf.layers.dense(sim_last, out_dim, kernel_initializer=tf.contrib.layers.xavier_initializer(), name="self"+str(i))
        #sim_last = sim_11
        att_input = sim_new+sim_11
        att_out = tf.layers.dense(att_input, 1, kernel_initializer=tf.contrib.layers.xavier_initializer(), name="att"+str(i))
        att_out = tf.nn.sigmoid(att_out)
